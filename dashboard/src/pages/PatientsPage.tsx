...
export default function PatientsPage() {
  ...
  const {data, isLoading, isError} = usePatientsPaginated(page, PAGE_SIZE);
  ...
  function openEdit(patient: PatientSummary) {
    setEditing(patient);
    setModalError('');
    setModalOpen(true);
  }
  ...
  return (
    <>
      <div style={styles.page}>
        ...
        <div style={styles.header}>
          <div>
            ...
            <p style={styles.subtitle}>
              {data ? `${data.total} registered patient${data.total !== 1 ? 's' : ''}` : ''}
            </p>
          </div>
        ...
        <div style={styles.tableWrap}>
          <table style={styles.table}>
            <thead>
              <tr>
                {['Patient', 'Total Scans', 'Adherence', 'Missed Doses', 'Last Scan', ''].map(
                  col => (
            ...
            <tbody>
              ...
              {data?.items.map(patient => (
                <tr
                  ...
                  {/* Patient column — click navigates to detail */}
                  <td
                    style={{...styles.td, cursor: 'pointer'}}
                    onClick={() => navigate(`/patients/${patient.id}`)}>
                    <div style={styles.emailText}>{patient.email}</div>
                    <div style={styles.idText}>
                      {patient.role === 'admin' ? '👑 Admin' : patient.id.slice(0, 8) + '…'}
                    </div>
                  </td>

                  <td style={styles.tdCenter}>{patient.total_scans}</td>

                  <td style={styles.tdCenter}>
                    <AdherencePill rate={patient.adherence_rate} />
                  </td>

                  <td style={styles.tdCenter}>{patient.missed_doses}</td>
      ...
      {deleteTarget && (
        <ConfirmDialog
          title="Delete patient?"
          message={`This will permanently delete ${deleteTarget.email} and all their scan history. This action cannot be undone.`}
          confirmLabel="Delete"
...
function AdherencePill({rate}: {rate: number}) {
  ...
  return (
    <div style={{display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'center'}}>
      ...
      <div style={{width: 60, height: 4, background: 'var(--bg-hover)', borderRadius: 4}}>
